...
__date__ = "$Date: 2014/08/12 $"
"""

""" ******************************************************************
This is the main file for the ParableW (Parable running on GUI)
...
2.0 - this is Parable Pi, meant to run stand-alone on a Raspberry Pi. This
file may be replaced with a non-gui main program file.
    ...
    def on_fileOpenBank_command(self, event):
        if (self.seq.running() == False):
            ...
            if result.accepted == True:
                ...
                tempofile=open(tempofn.replace("\\", "\\\\"), "r")
